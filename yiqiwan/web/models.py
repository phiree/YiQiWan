...
class Activity(models.Model):
    ...
    活动的定义.
    """
    founder = models.ForeignKey(settings.AUTH_USER_MODEL, related_name='activity_founder',
                                verbose_name=_lazy('founder'), blank=True,
    ...
    participants = models.ManyToManyField(settings.AUTH_USER_MODEL, related_name='activity_participants', null=True,
                                          blank=True)
    ...
    def get_each_pay(self, is_pre_pay, is_real_time):
        ...
        if is_pre_pay:
            ...
            total_amount_occur_actual = self.total_cost_actual + self.checkout_strategy.get_charge_amount(
                self.total_cost_actual)
    ...
    def checkout(self):
        ...
        amount_profit_founder = Decimal(amount_profit * self.checkout_strategy.founder_profit_percent).quantize(
            Decimal('.1'),
...
class Financial_Statement(models.Model):
    ...
    amount_for_founder = models.DecimalField(default=0, max_digits=6, decimal_places=1,
                                             help_text=_('profit for founder'))
...
def participant_checkout(participant, activity, amount, flow_type):
    ...
    # 如果没有离线账户 则创建
    try:
        balance_offline, created = User_User_Balance.objects.get_or_create(owner=participant,
                                                                           other_user=activity.founder)
    except MultipleObjectsReturned:
